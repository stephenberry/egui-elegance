...
> **Status:** shipped, and then expanded. This document proposed the first six action glyphs (`U+E00C`–`U+E011`). A follow-up batch added 17 more at `U+E012`–`U+E022` — a Status family (`info`, `triangle-alert`, `circle-x`, `circle-check`) that also rewires `CalloutTone`'s per-tone marks, plus broadly-useful nav/action icons (`settings`, `menu`, `arrow-left`/`right`, `external-link`, `chevron-right`/`down`) and situational ones (`funnel`→`FILTER`, `eye`, `eye-off`, `house`→`HOME`, `lock`, `save`). `folder` and `folder-open` landed later at `U+E023`–`U+E024`, on a downstream report of falling back to the 📁 emoji for a breadcrumb root — the emoji renders in the system emoji font, so it reads as a different icon set beside the Lucide glyphs next to it. The conventions and process below applied unchanged to all three.
